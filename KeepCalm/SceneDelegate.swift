...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private func notify(with payload: Payload, peer: Peer) {
        let content = UNMutableNotificationContent()
        content.body = "\"\(payload.message)\" from \(peer.name)"
        let request = UNNotificationRequest(identifier: payload.message.id.uuidString, content: content, trigger: nil)
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let contentView = ContentView(transceiver: self.transceiver).environmentObject(dataSource)
